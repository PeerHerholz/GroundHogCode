...
# env --> not mandatory, the environment the script should run in (e.g. if you normally work in python 3.*, you should source a python 2.*)
...
do
	...
	# define input image and set image names for subsequent processing
        ...
        IMAGE=$anat$extension # set complete name for processed, defaced, image
	      ...
	      cp $IMAGE $IMAGE_NODEFACE # rename original image
	...
	echo "working on $participant" # indicate which participant is currenly being processed
        ...
        pydeface $IMAGE_NODEFACE --outfile $IMAGE --force # apply pydeface on the original image, writing an output file named like the original image without the no_deface id
